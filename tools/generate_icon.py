...
def main():
    ...
    # Full-bleed rounded-square background (NO outline; avoid "frame" look)
    bg_layer = Image.new("RGBA", (size, size), BG)
    mask = rounded_rect_mask(size, radius)
    img = Image.composite(bg_layer, base, mask)

    # Very subtle highlight (so it doesn't look flat, but not a border)
    overlay = Image.new("RGBA", (size, size), (255, 255, 255, 0))
    od = ImageDraw.Draw(overlay)
    od.ellipse([-220, -300, size + 220, int(size * 0.52)], fill=(255, 255, 255, 45))
    img = Image.alpha_composite(img, overlay)

    d = ImageDraw.Draw(img)

    # Content: slightly smaller than before to create "normal icon" padding
    cx, cy = size // 2, size // 2 - 44

    # Soft circle backing (smaller)
    circle_r = 205  # was 230
    d.ellipse([cx - circle_r, cy - circle_r, cx + circle_r, cy + circle_r], fill=(255, 255, 255, 175))

    # Check mark (slightly thinner)
    d.line([(cx - 118, cy + 6), (cx - 26, cy + 110)], fill=PRIMARY, width=50, joint="curve")
    d.line([(cx - 26, cy + 110), (cx + 162, cy - 88)], fill=PRIMARY, width=50, joint="curve")

    # Progress bar (slightly smaller)
    bar_w = 520  # was 560
    bar_h = 50   # was 54
    bx1 = cx - bar_w // 2
    by1 = size - 235  # a bit higher for balance
    bx2 = bx1 + bar_w
    by2 = by1 + bar_h
    ...
    # IMPORTANT: do NOT draw any border/outline. (No frames.)
